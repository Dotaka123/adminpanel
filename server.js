...
async function getAuthToken() {
  ...
  const response = await axios.post(`${API_BASE_URL}/login`, {
    email: process.env.EMAIL,        // <--- TON email admin ici
...
const PRICES = {
  golden: {
    ...
    prices: [
      { duration: 0.02, price: 0.25 },
      { duration: 0.12, price: 0.45 },
      { duration: 1, price: 0.7 },
      { duration: 3, price: 2 },
      { duration: 7, price: 4 },
      { duration: 15, price: 7.5 },
      { duration: 30, price: 14.5 }
    ]
  },
  silver: {
    package_id: parseInt(process.env.SILVER_PACKAGE_ID) || 2,
    prices: [
      { duration: 2, price: 1.1 },
      { duration: 7, price: 3 },
